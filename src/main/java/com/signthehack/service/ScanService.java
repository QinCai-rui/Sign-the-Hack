...
import com.signthehack.config.AppConfig;
import com.signthehack.config.ConfigManager;
import com.signthehack.detection.CheckDefinition;
import com.signthehack.detection.CheckResult;
import com.signthehack.detection.CheckStatus;
import com.signthehack.detection.DetectionEvaluator;
...
public final class ScanService {
    ...
    public ScanService(JavaPlugin plugin, ConfigManager configManager, BiConsumer<ScanReport, CommandSender> completion) {
        this.plugin = plugin;
    ...
    public synchronized UUID startScan(CommandSender checker, Player target, List<String> requestedChecks, ScanReason reason) {
        if (active.containsKey(target.getUniqueId())) {
            checker.sendMessage("Scan already running for this player.");
            return null;
        }

        List<CheckDefinition> checks = resolveChecks(reason, requestedChecks);
        if (checks.isEmpty()) {
            checker.sendMessage("No checks selected.");
            return null;
        }
